	...
	<header id="header" class="animated slideInDown" :class="{ 'isScrolled': hasScrolled }" style="animation-delay:0.8s;">
		<table>
			<tr>
				...
				<td id="navigation" v-if="store.user_info">
					<img v-if="store.user_info.avatar" class="thign" :src="`https://cdn.discordapp.com/avatars/${store.user_info?.id}/${store.user_info?.avatar}.png`">
					<a class="no-effect">
...
const handleScroll = () => {
	if (window.innerWidth >= 1000) {
		hasScrolled.value = document.body.scrollTop > 80 || document.documentElement.scrollTop > 80;
	}
}
