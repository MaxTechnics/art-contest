        ...
        <div class="container space">
            <div class="content">
                <div class="grid" ref="grid">
                    <ArtItem v-for="item in submissions_shoufled" :key="item" :title="item.title" :description="item.description" :author="item.author" :imageURL="item.asset" :author_id="item.author_id" />
                </div>
                <div class="preview">
                    <button class="action action--close">
                        <i>
                            <svg class="action-svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512">
                    ...
                    <div class="description description--preview">
                        ...
                        <Button v-if="mainstore.user_status?.can_vote" @click="open_modal">I CHOOSE YOU!!!!</Button>
                    </div>
                </div>
            </div>
        </div>
...
import "@/assets/styles/ArtList/main.css";
import "@/assets/styles/ArtList/demo.css";
...
const grid = ref(null);
const show_vote_modal = ref(false);
const is_voting = ref(false);
const is_confirmed = ref(false);
const preview_author = ref(null);
const preview_img = ref(null);
const preview_id = ref(null);
const voting_finished = ref(false);
const vote_done_msg = ref('');
const vote_finished_title = ref('');
const maybe_wrong = ref(false);
...
const submissions_shoufled = shuffle(to_shou_fle);

const open_modal = () => {
    const data = preview_container.value.getElementsByClassName('non-confusing-name')[0];
    preview_author.value = data.getAttribute('data-author');
    preview_img.value = data.getAttribute('data-image');
    preview_id.value = data.getAttribute('data-authorid');
    show_vote_modal.value = true
}

const close_modal = () => {
    show_vote_modal.value = false;
    preview_author.value = null;
    preview_img.value = null;
    preview_id.value = null;
    is_voting.value = false;
    is_confirmed.value = false;
}

const vote_close_modal = () => {
    voting_finished.value = false;
    vote_done_msg.value = '';
    vote_finished_title.value = '';
    maybe_wrong.value = false;
}

const post_vote = () => {
    is_voting.value = true;
    fetch('/api/vote', {
        method: 'POST',
        body: JSON.stringify({ choice: preview_id.value }),
        ...
        if (response.status !== 200) {
            vote_finished_title.value = 'Your vote was not saved';
            vote_done_msg.value = data.message;
            maybe_wrong.value = data.maybe_wrong;
        } else {
            vote_finished_title.value = 'Vote saved!!';
            vote_done_msg.value = data.message;
            window.initBurst();
        }
        ...
        close_modal();
        vote_finished_title.value = 'Your vote was not saved';
        vote_done_msg.value = `The server didn't really do anything about the request, please contact Grady's Physics Homework (MaxTechnics) about it. (error: ${e})`;
        voting_finished.value = true;
    });
}
